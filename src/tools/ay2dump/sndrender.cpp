...
void SNDRENDER::end_frame(unsigned endframe_sys_tick)
{
        ...
        uint64_t endtick = ((passed_sys_ticks + endframe_sys_tick) * (uint64_t)sample_rate * TICK_F) / sys_clock_rate;
...
void SNDRENDER::set_timings(unsigned sys_clock_rate, unsigned sample_rate)
{
        ...
        mult_const = (unsigned) (((uint64_t)sample_rate << (MULT_C+TICK_FF)) / sys_clock_rate);
